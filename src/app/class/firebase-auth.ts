...
export class FirebaseAuth {
	...
	async getImages(returnObject, email) {
		...
		var array = new Array(); 
		this.bringEntityFilteredByDate("/image", returnObject, email);
		return;
...
*/
	...
	bringEntityFilteredByDate(path, returnObject, loggedUserEmail) {
		console.log("bringEntity");
		console.log("loggedUserEmail", loggedUserEmail);
		//var returnObject = new Array(); 
		...
		imageRef.snapshotChanges().forEach(snapshot => {
			...
			var postData = snapshot.forEach(doc => {
				...
				returnObject.push(postData);
				returnObject.sort(this.compareFecha);
			});
			console.log("after snapshto foreach");
	...
	bringEntityFilteredByDateAndOwner(path, returnObject, loggedUserEmail) {
		console.log("bringEntity");
		console.log("loggedUserEmail", loggedUserEmail);
		//var returnObject = new Array(); 
		...
		imageRef.snapshotChanges().forEach(snapshot => {
			...
			var postData = snapshot.forEach(doc => {
				...
				var flag = false;
				if(postData != null && postData != undefined){
					if(postData.user != loggedUserEmail)
						return;
